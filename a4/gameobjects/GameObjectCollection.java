...
public abstract class GameObjectCollection < T > extends GameObject implements Iterable < T > {
    ...
    public GameObjectCollection() { gameObjects = new CopyOnWriteArrayList < > (); }

    CopyOnWriteArrayList < T > getGameObjects() { return gameObjects; }

    public void add(T gameObject) { gameObjects.add(gameObject); }

    public void remove(T gameObject) { gameObjects.remove(gameObject); }

    public int getSize() { return gameObjects.size(); }
